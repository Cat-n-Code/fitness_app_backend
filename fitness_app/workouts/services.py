...
class WorkoutService:
    ...
    async def get_workouts_by_user(
        ...
        if user.role == Role.COACH:
            return await self._workout_repository.get_workouts_by_coach_id(
                session, user.coach_info.id, page, size, date_start, date_finish
            )
        elif user.role == Role.CUSTOMER:
            return await self._workout_repository.get_workouts_by_customer_id(
                session, user.customer_info.id, page, size, date_start, date_finish
            )
